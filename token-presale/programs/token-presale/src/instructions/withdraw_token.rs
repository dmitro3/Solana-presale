...
pub fn withdraw_token(ctx: Context<WithdrawToken>, identifier: u8) -> Result<()> {
    ...
    msg!(
        "From Token Address: {}",
        &ctx.accounts.presale_presale_token_associated_token_account.key()
    );
    ...
    token::transfer(
        CpiContext::new_with_signer(
            ...
            &[
                &[
                    PRESALE_SEED,
                    ctx.accounts.presale_authority.key().as_ref(),
                    [identifier].as_ref(),
                    bump,
                ][..],
            ]
