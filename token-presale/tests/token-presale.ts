...
describe("token_presale", () => {
  ...
  const tokenTitle = "KingAI";
  const tokenSymbol = "KAI";
  const tokenUri = "https://bafkreihfx7pt4j3ep6cjyb3e4st5sqgq4vfovgebztwhqmipj2ubfb2bmu.ipfs.nftstorage.link";
  const quoteTokenTitle = "SOL";
  ...
  const bABTokenPubkey = new PublicKey("5hQoVfgNs5JvSyyZipx2qEH2mZJbceQ5o8SyFTrJzk7R");
  const quoteTokenPubkey = new PublicKey("So11111111111111111111111111111111111111112");
  const solTokenPubkey = new PublicKey("So11111111111111111111111111111111111111112");
  const usdcTokenPubkey = new PublicKey("4zMMC9srt5Ri5X14GAgXhaHii3GnPAEERYPJgZJDncDU");
  const usdtTokenPubkey = new PublicKey("EJwZgeZrdC8TXTQbQBoL6bfuAnFUUy1PVCMB4DYPzVaS");
  const jupTokenPubkey = new PublicKey("EJwZgeZrdC8TXTQbQBoL6bfuAnFUUy1PVCMB4DYPzVaS"); // devnet no
  ...
  const getUserPDA = async () => {
    return (
      await PublicKey.findProgramAddressSync(
        [Buffer.from(USER_SEED), myPubkey.toBuffer()],
        PROGRAM_ID
      )
    )[0];
  };
  ...
  it("Presale account 0 is initialized!", async () => {
    ...
    const walletPDA = await getUserPDA();
    const presalePDA = await getPresalePDA( 0 );
    console.log(`Wallet address: ${walletPDA}`);
    console.log(`Presale address: ${presalePDA}`);
    
    const tx = await program.methods
      .createPresale( bABTokenPubkey, solTokenPubkey, usdtTokenPubkey, usdcTokenPubkey, jupTokenPubkey, TEN, ONE, TWO )
      .accounts({
